...
def main() -> None:
    ...
    stale = [
        source for source in steering_sources
        if not valid_steering_artifact(source, fingerprint)
    ]
    if stale:
        raise RuntimeError(
            f"{len(stale)} steering traces were generated by an old probe or "
            "configuration; rerun 07d before finalizing"
        )
