...
It records ordinary harness-level functional correctness, a raw strict
completion-only result, and a normalized result. The strict result additionally
requires an indented continuation of the supplied HumanEval function and
rejects repeated function definitions, generation markers, special tokens, and
Markdown fences. For standalone programs, the deterministic AST normalizer
nests imports/helpers and a renamed generated entry function inside the supplied
function, then calls it with the supplied arguments. Recursion is renamed with
the wrapper, and the normalized continuation is executed against the same tests.
Stage 07e keeps raw strict accuracy as the commercial gate and prints functional
and normalized accuracy alongside it; normalization never silently replaces the
original metric.
